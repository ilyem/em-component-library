...
const Label = styled(props => <Text tag="label" {...props} />)` 
  color: ${({ $error, theme }) => ($error ? theme.colours.error : theme.colours.primary)};
...
const Input = styled.input<{ $error: boolean, theme: ThemeType }>`
  && {
    ...
    width: 100%; 
    border-bottom: ${({theme}) => theme.border.size} solid ${({theme}) => theme.colours.border};
    height: 35px;
    ...
    ${({ $error, theme }) => ($error ? `border-color: ${theme.colours.error};` : '')}
  &:disabled {
      cursor: not-allowed;
...
const Error = styled(Text)`
  margin-top: ${({theme}) => theme.spacing.eightPx};
`;
